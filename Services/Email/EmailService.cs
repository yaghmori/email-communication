...
public interface IEmailService
{
    ...
    Task<bool> SendEmailWithRetryAsync(EmailPayload payload, string? tenantId = null, int maxRetries = 3);
    Task<bool> SendEmailAsync(EmailPayload payload, string mode, string? tenantId = null);
    Task<bool> SendEmailWithRetryAsync(EmailPayload payload, string mode, string? tenantId = null, int maxRetries = 3);
}
...
public class EmailService : IEmailService
{
    ...
    private readonly EmailTcpClient? _tcpClient;
    ...
    )
    {
        ...
        _logger = logger;
        _mode = configuration["EmailService:Mode"] ?? "Tcp";
    }
    ...
    )
    {
        for (int attempt = 1; attempt <= maxRetries; attempt++)
        {
            ...
            if (success)
            {
                if (attempt > 1)
                {
                    _logger.LogInformation("Email sent successfully after {Attempt} attempts", attempt);
                }
                return true;
            }
        ...
        _logger.LogError("Failed to send email after {MaxRetries} attempts", maxRetries);
        return false;
    }
    ...
    private async Task<bool> SendViaKafkaAsync(EmailPayload payload, string? tenantId)
    {
        ...
        return await _kafkaPublisher.PublishAsync(payload, tenantId);
    }
    ...
    private async Task<bool> SendViaTcpAsync(EmailPayload payload, string? tenantId)
    {
        ...
        var response = await _tcpClient.SendAsync(payload);
